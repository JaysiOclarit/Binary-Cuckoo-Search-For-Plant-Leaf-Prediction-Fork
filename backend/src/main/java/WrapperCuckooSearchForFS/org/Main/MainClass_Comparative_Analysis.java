...
/**
 * Main Execution Pipeline for Comparative Analysis between Baseline BCS and
 * Proposed GBCS.
 * <p>
 * Evaluates both feature selection algorithms under identical dataset split
 * conditions and
 * compares accuracy, precision, recall, F1-score, feature reduction ratio
 * (FRR), average
...
public class MainClass_Comparative_Analysis {
        ...
        public static void main(String[] args) throws IOException {
                String dataPath = Paths.get("Entire Data Folder", "Original Dataset", "Swedish Leaf data.csv")
                                .toString();
                System.out.println("=================================================");
                ...
                // Shared FM + Bagging Trainer (10 weak learners) for final classification
                // evaluation
        ...
        private static void printAndSaveComparison(AlgorithmResult bcs,
                ...
                report.append(String.format("%-32s | %-20s | %-20s\n", "Metric / Evaluation Parameter", "Baseline BCS",
                                "Proposed GBCS (Ours)"));
                report.append("---------------------------------------------------------------------------------\n");
                report.append(String.format("%-32s | %-20s | %-20s\n", "FS Execution Time",
                                Util.formatDuration(0, bcs.fsDurationMs()),
                                Util.formatDuration(0, gbcs.fsDurationMs())));
                report.append(String.format("%-32s | %-20d | %-20d\n", "Original Dataset Features",
                                bcs.totalOriginalFeatures(), gbcs.totalOriginalFeatures()));
                report.append(String.format("%-32s | %-20d | %-20d\n", "Selected Feature Subset Size",
                                bcs.selectedFeatures(), gbcs.selectedFeatures()));
                report.append(String.format("%-32s | %-20.2f%% | %-20.2f%%\n", "Feature Reduction Ratio (FRR)",
                                bcs.reductionRatioPercent(), gbcs.reductionRatioPercent()));
                report.append(String.format("%-32s | %-20.4f | %-20.4f\n", "Avg Feature Correlation (rho_avg)",
                                bcs.averageCorrelation(), gbcs.averageCorrelation()));
                report.append(String.format("%-32s | Iteration %-10d | Iteration %-10d\n",
                                "Optimal Convergence Iteration", bcs.optimalConvergenceIteration(),
                                gbcs.optimalConvergenceIteration()));
                report.append("---------------------------------------------------------------------------------\n");
                report.append(String.format("%-32s | %-20.2f%% | %-20.2f%%\n", "Test Classification Accuracy",
                                bcs.accuracy(), gbcs.accuracy()));
                report.append(String.format("%-32s | %-20.2f%% | %-20.2f%%\n", "Macro-Averaged Precision",
                                bcs.precision(), gbcs.precision()));
                report.append(String.format("%-32s | %-20.2f%% | %-20.2f%%\n", "Macro-Averaged Recall", bcs.recall(),
                                gbcs.recall()));
                report.append(String.format("%-32s | %-20.2f%% | %-20.2f%%\n", "Macro-Averaged F1-Score", bcs.f1Score(),
                                gbcs.f1Score()));
                report.append("=================================================================================\n\n");
                ...
                try (PrintWriter csv = new PrintWriter(new FileWriter("Comparative_Results.csv"))) {
                        ...
                        csv.printf("Precision,%.4f,%.4f,%+.4f\n", bcs.precision(), gbcs.precision(),
                                        gbcs.precision() - bcs.precision());
                        csv.printf("Recall,%.4f,%.4f,%+.4f\n", bcs.recall(), gbcs.recall(),
                                        gbcs.recall() - bcs.recall());
                        csv.printf("F1Score,%.4f,%.4f,%+.4f\n", bcs.f1Score(), gbcs.f1Score(),
                                        gbcs.f1Score() - bcs.f1Score());
                        csv.printf("SelectedFeatures,%d,%d,%d\n", bcs.selectedFeatures(), gbcs.selectedFeatures(),
                                        gbcs.selectedFeatures() - bcs.selectedFeatures());
                        csv.printf("ReductionRatioPercent,%.2f,%.2f,%+.2f\n", bcs.reductionRatioPercent(),
                                        gbcs.reductionRatioPercent(), frrDiff);
                        csv.printf("AverageCorrelation,%.4f,%.4f,%+.4f\n", bcs.averageCorrelation(),
                                        gbcs.averageCorrelation(), -corrDiff);
                        csv.printf("OptimalConvergenceIteration,%d,%d,%d\n", bcs.optimalConvergenceIteration(),
                                        gbcs.optimalConvergenceIteration(), -speedupIter);
                }

                System.out.println(
                                "SUCCESS: Results saved to 'BCS_vs_GBCS_Comparison_Report.txt' and 'Comparative_Results.csv'");
        }
}
