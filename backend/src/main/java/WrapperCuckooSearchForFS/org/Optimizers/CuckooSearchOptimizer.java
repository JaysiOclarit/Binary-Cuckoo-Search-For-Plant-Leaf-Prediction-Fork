...
/**
 * Select features based on Cuckoo Search algorithm with binary transfer
 * functions, KNN classifier and 10-fold cross validation
 * <p>
 * see:
 * 
 * <pre>
...
public final class CuckooSearchOptimizer implements FeatureSelector<Label> {
    ...
    /**
     * The default constructor for feature selection based on Cuckoo Search
     * Algorithm
     */
     ...
     * @param transferFunction The transfer function to convert continuous values to
     *                         binary ones
     * @param populationSize   The size of the solution in the initial population
     * @param maxIteration     The number of times that is used to enhance
     *                         generation
     * @param seed             This seed is required for the SplittableRandom
     */
    public CuckooSearchOptimizer(Trainer<Label> trainer, TransferFunction transferFunction, int populationSize,
            int maxIteration, int seed) {
     ...
     * 
     * @param trainer              The used trainer in the evaluation process
     * @param transferFunction     The transfer function to convert continuous
     *                             values to binary ones
     * @param populationSize       The size of the solution in the initial
     *                             population
     ...
     * @param delta                The delta that is used in the abandon nest
     *                             function
     * @param maxIteration         The number of times that is used to enhance
     *                             generation
     * @param seed                 This seed is required for the SplittableRandom
     */
    public CuckooSearchOptimizer(Trainer<Label> trainer, TransferFunction transferFunction, int populationSize,
            double stepSizeScaling, double lambda, double worstNestProbability, double delta, int maxIteration,
            int seed) {
        this.transferFunction = transferFunction;
        this.populationSize = populationSize;
        FN = new FitnessFunction(trainer);
    ...
    /**
     * Selects features according to this selection algorithm from the specified
     * dataset.
     * 
    ...
    public SelectedFeatureSet select(Dataset<Label> dataset) {
        ...
        for (int i = 0; i < maxIteration; i++) {
            ...
            for (int solution = 0; solution < setOfSolutions.length; solution++) {
                ...
                int[] evolvedSolution = Arrays.stream(setOfSolutions[subSet.get()])
                ...
                if (rng.nextDouble() < worstNestProbability) {
                    ...
                    for (int j = 0; j < setOfSolutions[subSet.get()].length; j++) {
                        evolvedSolution[j] = (int) transferFunction.applyAsDouble(setOfSolutions[subSet.get()][j] +
            ...
            Arrays.stream(setOfSolutions).map(subSet -> new CuckooSearchFeatureSet(subSet,
                    FN.EvaluateSolution(this, dataset, FMap, subSet))).forEach(subSet_fScores::add);
            subSet_fScores.sort(Comparator.comparing(CuckooSearchFeatureSet::score).reversed());
    ...
    /**
     * This record is used to hold subset of features with its corresponding fitness
     * score
     */
